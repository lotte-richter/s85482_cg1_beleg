...
enum VAO_IDs { VAOSquare, VAOCircle, VAOCube, VAOCone, NumVAOs };
enum Buffer_IDs { ArrayBufferSquare, ArrayBufferCircle, ArrayBufferCube, ArrayBufferCone, NumBuffers };
...
void generateCube() {
    GLfloat vertices[][3] = {
        {-0.90,-0.90,0.9},{0.90,-0.90,0.9},{0.90,0.90,0.9},{-0.90,0.90,0.9},
        {0.90,-0.90,0.9},{0.90,-0.90,-0.9},{0.90,0.90,-0.9},{0.90,0.90,0.9},
        {0.90,-0.90,-0.9},{0.90,0.90,-0.9},{-0.90,0.90,-0.9},{-0.90,-0.90,-0.9},
        {-0.90,-0.90,-0.9},{-0.90,0.90,-0.9},{-0.90,0.90,0.9},{-0.90,-0.90,0.9},
        {-0.90,-0.90,0.9},{0.90,-0.90,0.9},{0.90,-0.90,-0.9},{-0.90,-0.90,-0.9},
...
//generateCone und drawCone von Christoph Born
void generateCone(float radius = 0.90, float startAngle = 0.00, float height = 0/*, int circPoints = 20*/) {
    if (height == 0)  height = radius * 2;

    const int circPoints = 20; // otherwise error on 2D-array declaration
    const float sectionAngle = PI * 2 / circPoints;

    // base
    GLfloat vertices[(circPoints + 2) * 2][3] = { {0.00, 0.00, 0.00} };//TODO FLÄCHE VERKEHRT RUM

    for (int i = 0; i < circPoints; i++) {
        vertices[i + 1][0] = cos(sectionAngle * i + startAngle) * radius;
        vertices[i + 1][1] = 0.00;
        vertices[i + 1][2] = sin(sectionAngle * i + startAngle) * radius;
    }

    vertices[circPoints + 1][0] = vertices[1][0];
    vertices[circPoints + 1][1] = vertices[1][1];
    vertices[circPoints + 1][2] = vertices[1][2];

    // outer
    vertices[circPoints + 2][0] = 0.00;
    vertices[circPoints + 2][1] = height;
    vertices[circPoints + 2][2] = 0.00;

    for (int i = 0; i < circPoints; i++) {
        vertices[circPoints + 2 + i + 1][0] = cos(sectionAngle * i + startAngle) * radius;
        vertices[circPoints + 2 + i + 1][1] = 0.00;
        vertices[circPoints + 2 + i + 1][2] = sin(sectionAngle * i + startAngle) * radius;
    }

    vertices[circPoints + 2 + circPoints + 1][0] = vertices[circPoints + 2 + 1][0];
    vertices[circPoints + 2 + circPoints + 1][1] = vertices[circPoints + 2 + 1][1];
    vertices[circPoints + 2 + circPoints + 1][2] = vertices[circPoints + 2 + 1][2];

    glBindVertexArray(VAOs[VAOCone]);
    glBindBuffer(GL_ARRAY_BUFFER, Buffers[ArrayBufferCone]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
    glVertexAttribPointer(vPosition, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);
    glEnableVertexAttribArray(vPosition);
}
void drawCone(int colorR = 1, int colorG = 1, int colorB = 0/*, int circPoints = 20*/) {
    int circPoints = 20;

    glBindVertexArray(VAOs[VAOCone]);
    glVertexAttrib3f(vColor, colorR, colorG, colorB);
    //glEnable(GL_CULL_FACE); //Rückseitenentfernung
    glCullFace(GL_BACK);
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glDrawArrays(GL_TRIANGLE_FAN, 0, circPoints + 2);
    glDrawArrays(GL_TRIANGLE_FAN, circPoints + 2, circPoints + 2);
}

// ==== base program ===
void init() {
    program = loadShaders("Programme/Wuerfel.vs", "Programme/transparenz.fs", "", "", "", "");
...
void display() {
    ...
    glCullFace(GL_FRONT);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); //typische Transparenz
