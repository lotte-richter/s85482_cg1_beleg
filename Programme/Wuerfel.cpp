...
GLint height = 800, width = 800;
enum VAO_IDs { VAOSquare, VAOCircle, VAOCube, NumVAOs};
enum Buffer_IDs { ArrayBufferSquare, ArrayBufferCircle, ArrayBufferCube, NumBuffers };
...
void generateCone() {
    ...
    float r = 0.9, sectionAngle = 2 * PI / NumVertices;
    GLfloat vertices1[NumVertices + 2][2];
    GLfloat vertices2[NumVertices + 2][2];

    for (int i = 0; i < NumVertices; i++) {
        vertices1[i + 1][0] = cos(sectionAngle * i) * r;
        vertices1[i + 1][1] = sin(sectionAngle * i) * r;
    }

    vertices1[NumVertices + 1][0] = vertices1[1][0];
    vertices1[NumVertices + 1][1] = vertices1[1][1];
}
void drawCone() {
    int NumVertices = 20;
    glBindVertexArray(VAOs[VAOCircle]);
...
// ==== base program ===
void init() {
    program = loadShaders("Programme/Wuerfel.vs", "Programme/Wuerfel.fs", "", "", "", "");
